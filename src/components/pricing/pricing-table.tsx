          ...
          return (
            <div
              ...
              className={cn(
                ...
                isPopular && isLanding && "border-2 border-emerald-500/60 bg-gradient-to-b from-emerald-900/30 to-white/60",
            ...
            >
              {isPopular && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                  <span className="px-3 py-1 bg-emerald-600 text-gray-900 text-xs font-semibold rounded-full">
                    Populaire
                  </span>
                </div>
              )}
              {isCurrent && !isPopular && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                  <span className="px-3 py-1 bg-gray-700 text-gray-900 text-xs font-semibold rounded-full">
                    Plan actuel
                  </span>
                </div>
              )}
              ...
              <div className="mb-6">
                <h3 className={cn("text-lg font-bold mb-1", isLanding ? "text-gray-900" : "text-gray-900")}>
                  {plan.name}
                </h3>
                <p className={cn("text-sm mb-4", isLanding ? "text-gray-500" : "text-gray-500")}>
                  {plan.description}
                </p>
                <div className="flex items-baseline gap-1">
                  <span className={cn("text-4xl font-bold", isLanding ? "text-gray-900" : "text-gray-900")}>
                    {plan.price === 0 ? "0 €" : `${plan.price} €`}
                  </span>
                  {plan.price > 0 && (
                    <span className={cn("text-sm", isLanding ? "text-gray-500" : "text-gray-500")}>/mois</span>
                  )}
                </div>
                {plan.priceAnnual > 0 && (
                  <p className={cn("text-xs mt-1", isLanding ? "text-gray-9000" : "text-gray-400")}>
              ...
              <ul className="space-y-2.5 mb-6 flex-1">
                {plan.features.map((feature) => (
                  <li key={feature} className="flex items-start gap-2">
                    ...
                    <span className={cn("text-sm", isLanding ? "text-gray-700" : "text-gray-700")}>
                      {feature}
                    </span>
                  </li>
                ))}
                ...
                <Link href={plan.id === "FREE" ? "/register" : `/register?plan=${plan.id}`}>
                  <Button
                    className={cn(
                      "w-full",
                      isPopular
                        ? "bg-emerald-600 hover:bg-emerald-700 text-gray-900"
                        : plan.id === "FREE"
                ...
                <div className="space-y-2">
                  {isCurrent ? (
                    ...
                    <Button
                      className={cn(
                        "w-full",
                        isPopular ? "bg-emerald-600 hover:bg-emerald-700 text-gray-900" : ""
                      )}
                      variant={isPopular ? "default" : "outline"}
      ...
      {variant === "landing" && (
        <div className="grid sm:grid-cols-3 gap-4 mt-4">
          {[
            ...
            <div key={q} className="rounded-xl bg-white/40 border border-gray-300/60 p-4">
